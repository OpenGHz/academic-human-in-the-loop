...
### Step 4: Output
Present as a structured literature table (canonical columns — the SAME set used in the
persisted file, Step 5; group rows by theme with a `###` sub-heading per theme when the set
is large):

```
| Paper | Venue/Yr | Method | Relevance to Us | Status |
|-------|----------|--------|-----------------|--------|
```

`Status` carries the Step-1.5 verification tag (`✅ verified (via arxiv|crossref|s2)` /
`⚠️ UNVERIFIED (...)` / `… verify_pending`) — never silently drop a non-verified paper.
When provenance matters (mixed Zotero/local/web hits) you MAY append a `Source` column.
...
- **Path**: `literature/related_work.md` (create `literature/` if absent). If a review for a
  *different* topic already exists at that path, write to
  ...
  which path you used.
- **Contents** = the full Step 4 deliverable, persisted using the **canonical file template**
  below (stable headings so downstream skills can locate fields deterministically).
  **Verified entries only** in the bib — never fabricate an arXiv ID/DOI for a
  `verify_pending`/`UNVERIFIED` paper; list those under *Pending / Unverified* so the user can audit.
...
**Canonical file template.** Use these exact section headings and order so downstream skills
can locate fields without guessing — `/novelty-check` patches *Novelty & Positioning Updates*,
`/title-decider` reads *Closest Prior Work & Delta* and *Naming / Terminology Collision Risks*,
`/idea-creator` folds the landscape into `IDEA_REPORT.md`. Keep a heading even when a section is
N/A (write "none found" rather than deleting it), so a missing section never reads as "not searched".

```markdown
# Literature Landscape — <Topic>

> One-line topic statement. Source idea: <path>. Generated by `/research-lit` on <YYYY-MM-DD>.
> Sources: <list>. Verification: <X/Y arXiv-verified, hallucination rate Z; N pending>.

## Scope & Decomposition
<How the idea breaks into components / sub-questions, to map prior work against. Recommended.>

## Literature Table
<Papers grouped by theme — one `###` sub-heading per theme. Canonical columns:>
<| Paper | Venue/Yr | Method | Relevance to Us | Status |>

## Synthesis
<3–5 paragraphs: the landscape, themes, consensus vs disagreement.>

### Closest Prior Work & Delta
<The 2–4 nearest works; for each, the precise differentiator vs the user's idea. Consumed by
 /title-decider and /novelty-check — keep it explicit, not buried in prose.>

### Gaps This Idea Fills
<Numbered, defensible contributions the landscape leaves open.>

### Naming / Terminology Collision Risks
<Terms whose existing meaning collides with the idea's naming/framing. Consumed by /title-decider.>

### Suggested Baselines & Evaluation
<Optional: baseline ladder mapped to cited works; metrics reviewers will expect.>

## Novelty & Positioning Updates
<Append-target for later /novelty-check & /title-decider passes: dated "Round-N" notes,
 closer competitors found, claim re-scoping. Empty on first write.>

## references.bib
<A bibtex code block — VERIFIED entries only.>

## Pending / Unverified
<Papers that did not verify this run — title + reason. Never emit a fabricated ID here.>
```
