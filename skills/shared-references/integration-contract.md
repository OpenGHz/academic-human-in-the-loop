...
| `paper_video.py` (`preflight`, `assemble`, `verify`, `package`) | A (skill-local gate) | `/paper-video` cannot produce a submission-ready MP4 without these; `verify` exit 2 blocks the submission-ready claim (size > MAX_MB, duration > MAX_SECONDS, wrong codec, missing faststart). Canonical location is `skills/paper-video/scripts/paper_video.py` (Arch C, self-contained — no `tools/` shim). |
...
| paper-video submission gate | `paper_video.py preflight --workspace <cwd> --venue <V>` returns `ok=true` | `paper_video.py` (`preflight`, `assemble`, `verify`, `package`) | `submission/video/supplementary.mp4`, `manifest.json`, `verify.json` (+ optional `supplementary.zip`) | Step 0 checklist in `paper-video` | `paper_video.py assemble --manifest <m> --output <mp4>` | `paper_video.py verify --venue <V>` (skill-local gate; exit 2 blocks submission-ready claim on size / duration / codec / faststart violation) |
