...
7. **`../embodied-ai-paper-writer/SKILL.md`** (only when the paper is robotics / embodied-AI) — domain-specific writing-craft coach distilled from 63 top robotics papers (CoRL, RSS, ICRA, IROS, Science Robotics). Triggered when the resolved venue is `CORL` or RSS/ICRA/IROS/Humanoids/ISRR, or when PAPER_PLAN.md / NARRATIVE_REPORT.md mentions robot, manipulation, locomotion, navigation, sim2real, embodied agent, VLA, real-robot experiments, teleoperation, etc. Use it for vocabulary, sentence patterns, paragraph flow, section openers, figure/table caption conventions, and rhetorical pivots specific to robotics venues. **It is a style/craft reference, not a content advisor** — it does NOT decide what to claim. Compose its guidance with `../shared-references/writing-principles.md` (this skill's defaults still apply); when the two conflict, the embodied-AI guidance takes precedence for craft-level decisions (e.g., section opener patterns), the shared-references take precedence for cross-skill protocols (e.g., em-dash budget).
